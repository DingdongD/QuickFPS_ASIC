...
class _DiscardingEventList(list):
    """List-compatible sink used when full event traces are disabled."""
...
class SummaryStrictClosedLoopQuickFPSCycleModel(ClosedLoopQuickFPSCycleModel):
    """Strict closed-loop simulator that can run without storing event objects.

    The scheduler is equivalent to the synchronous bucket-SRAM strict model,
    while the Point-Engine uses the resident-buffer, independent-DMA, NumPy,
    and batched-DRAMsim3 optimizations.  When ``trace_events`` is false, both
    the scheduler and Point-Engine append into a discarding sink instead of
    constructing a multi-million-entry trace list.
    """
    ...
    def run(self) -> SimulationResult:
        ...
        scheduler_name = "closed-loop-strict-pre-edge-sram-optimized"
        if self.sample_count == 1:
            return SimulationResult(
                ...
                events=[] if not self.trace_events else events,
                config={
        ...
        while cycle < self.accelerator.max_cycles:
            ...
            raise TimeoutError(
                f"optimized strict closed-loop QuickFPS exceeded "
